...
规划文档按版本迭代，**只看最新一版**。演进链：v1（微服务）→ v2（自研单体/PG）→ v3（RuoYi 底座，主库未定）→ v3.1（RuoYi + MySQL 定案）→ v3.2（辨析国标区间 vs AVC 控制目标）→ v3.3（统计单元/桶，**已撤销**）→ v3.4（撤销桶）→ v4.0（自洽整合 + 算法搁置 + 前端并入）→ v4.1（修订待办全量合并——三阶段管线 + 判定参数表 + 流水账 D8）→ **v5.0（现行权威：v4.1 + 策略解耦设计——数据不可用策略参数化、与算法分轨解封、新增 D9）**。v5.0 把 v3.1 底座、v3.4 管理表、草稿4_1 判定口径、前端三阶段、搁置轨工程决策及策略解耦设计整合成一份自洽落地依据。
...
| **AVC考核核心算法_草稿4_1.md** | `docs/AVC考核核心算法_草稿4_1.md` | ✅ **算法口径权威**（2026-08-17 增订版 = 原草稿 + 拍板回写：两态判定/免考后置读 yx501、T_fast∈[1,5) 默认 4/T_econ=5 写死、投运率信号点位补齐；**原 `AVC考核核心算法_草稿.md` superseded 存档、不再修改**）。含投运率(§一)+调节合格率(§二)+两档平行模型(§2.4–2.7)。草稿自述仍待真实数据定稿，但**判定口径以此为准**。v5.0 §8 把判定实现搁置为接口+stub |
...
- **关键**：判定抽象为接口（输入=指令+窗口逐分钟 high_SV/low_SV+参数，输出=**两档两态**结论 `{QUALIFIED, PENALIZED}` + **判不了信号 `Undecodable{reason}`（v5.0 新增，输入有效性信号、非第三判定态）**——免考 = 后置读对端 `yx501` 的应用，非判定产物；整体为「门控前置过滤 → judge 包络判定 → 免考后置应用」三阶段管线），算法定型后**只换实现、不动调用方**（解码留在 judge 实现内）。详见 v5.0 §8/§12。**没定 DDL 前别硬写统计表**——留空壳接口即可。
...
- **✅ 数据不可用策略搁置（2026-08-15 Leo 拍板）**：#3 缺数据 / #5 解码失败**不再要求先行定稿**，随搁置轨（判定实现 + 统计 DDL + Quartz）同批解封时定；搁置期原始事实（指令原文）落 `vqms_command_ledger`，`undecodable_count`/缺分钟数解封后从流水重算（v5.0 §6.2.6/§8.6）。详见 v5.0 §8.6 / `docs/数据不可用处理策略.md`（已标搁置轨）。
...
- **后端落地**：`ruoyi-vqms` 模块、build/test 命令、真实数据验证均未开始（确定轨可立即推进 D1~D9，见 v5.0 §12.1；**D1 目标本周内（2026-08-23 前）启动**，Leo 2026-08-18 定）。
