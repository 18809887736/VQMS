...
- **✅ 数据不可用策略搁置（2026-08-15 Leo 拍板）**：#3 缺数据 / #5 解码失败**不再要求先行定稿**，随搁置轨（判定实现 + 统计 DDL + Quartz）同批解封时定；搁置期原始事实（指令原文）落 `vqms_command_ledger`，`undecodable_count`/缺分钟数解封后从流水重算（v5.0 §6.2.6/§8.6）。详见 v5.0 §8.6 / `docs/数据不可用处理策略.md`（已标搁置轨）。**免考旗（yx501）读取失败为第三类同构失效（2026-08-19 Leo 拍板：随 #3/#5 同批分轨登记，从严/从宽/挂起处置待定，策略文档 §1/§6 已增补）**。
