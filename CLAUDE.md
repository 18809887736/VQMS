...
- **✅ 数据不可用策略搁置（2026-08-15 Leo 拍板）**：#3 缺数据 / #5 解码失败**不再要求先行定稿**，随搁置轨（判定实现 + 统计 DDL + Quartz）同批解封时定；搁置期发令次数 / `undecodable_count` / 窗口缺分钟数**照实记录、只记不判**。详见 v4.1 §8.6 / `docs/数据不可用处理策略.md`。
