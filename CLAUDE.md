...
- **Per-minute verdict** uses `average_SV` against a **static per-busbar qualification interval** `[low_limit, high_limit]` — NOT the row's `low_SV`/`high_SV`: `low_limit≤avg≤high_limit` = qualified, `avg>high_limit` = over-high, `avg<low_limit` = over-low. Boundary equality counts as qualified. **Threshold source is an OPEN decision** (`BUSBAR_VRateParameter` in the SQLite config DB, vs. a VQMS-managed table) — blocks the verdict impl until resolved; see `md/外部数据源.md` §5.
- **`plan_SV` = AVC dispatch setpoint**, but the sample value `10245` is **obviously unreasonable** (actual voltage fields sit at 230~300, ~40× off) — unusable until `ori_code`-decoded; that table is currently **empty**, so treat as a placeholder and do not display as a real voltage. Details: `md/外部数据源.md` §4.
