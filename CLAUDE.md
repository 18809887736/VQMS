...
**规划 + 前端骨架 + 后端确定轨 D1~D5 阶段。** RuoYi 脚手架已就位（`RuoYi-Vue-springboot3/` 后端、`RuoYi-Vue3/` 前端），VQMS 前端业务页面骨架已落地（`RuoYi-Vue3/src/views/vqms/` 下 7 个页面：curve / daily / monthly / yearly / avc-runtime / avc-regulation / threshold）；后端已落地 D1~D5（2026-08-21）：D1 source 只读层、D2 管理表 + 逻辑 FK、D3 时间对齐工具、D4 三步闸门 + pre-commit L0 检查点、D5 控制器/菜单/perms 对接（报告见 `docs/测试/D1~D5_*.md`）。build/test：`JAVA_HOME=<JDK17> mvn -pl ruoyi-vqms,ruoyi-admin -am test`（本机默认 JDK 1.8 须显式指 `/c/environment/jdk-17.0.20+8`；`*IT` 集成测试不随默认 `test` 跑，须 `-Dtest=xxxIT` 且 10.0.0.9 可达——管理侧 @13306 需 `MYSQL_ROOT_PASSWORD`，外部源侧 @3306 需 `VQMS_AVC_TEST_USER/PASSWORD`；pre-commit hook 已启用 `core.hooksPath=.githooks`，覆盖两模块 L0）。
...
- **后端落地**：✅ D1~D5 已交付（2026-08-21，source 只读层 / 管理表 + 逻辑 FK / 时间对齐工具 / 三步闸门 + pre-commit L0 检查点 / 控制器与菜单 perms 对接，各带测试与报告）；D6~D9 未开始，下一个 = D6 前端阶段 3（真实数据接入 + L3 冒烟：403 断言、浏览器端到端、busbar 下拉切接口、curve 去 mock/average 文案、threshold vGrade join）。两枚测试口令待轮换（主库 root @13306 + 测试库 root @3306，历史值在 git 历史）。真实数据验证仍未做（等现场样本）。
