...
- **后端落地**：✅ **确定轨 D1~D9 已全部交付**（2026-08-22，source 只读层 / 管理表 + 逻辑 FK / 时间对齐工具 / 三步闸门 + pre-commit L0 检查点 / 控制器与菜单 perms 对接 / 前端真实数据接入 + L3 冒烟 / 判定参数 CRUD + CHECK 双层 + Redis 缓存 / 指令流水账抓取入库 + 生成列 uk 幂等 / 类型化失败契约 sealed RegulationOutcome + StubRegulationJudge（分类真·结论占位）+ DataUnavailabilityPolicy 四桶纯函数 + vqms_policy_param 表（零种子选套留空），增量迁移均已对 ry_vqms 执行，各带测试与报告）；**下一步 = 搁置轨推进（算法已定稿 2026-08-22：S1~S4 解封可开工——✅ S2/S3 纯函数半边均已交付 2026-08-24（ExemptionApplier/RegulationStatistics + RuntimeMinuteClassifier/RuntimeStatistics，落库 DDL+rollup 半边待设计）；✅ S1 判定实现已交付 2026-08-24（六组件全量：Decoder/Aggregator/DefaultRegulationJudge/注册表/YxSignalReader/GateFilter；⚠️ 契约准入 IT 待口令跑批 + GateFilter 置 1 联调随真实点号核对；stub 护栏 isStub() 拒入正式统计随 S4 管线补）；✅ S2/S3 统计落库 DDL 已定稿落地 2026-08-24（设计稿六项决策 Leo「1-6 全做」全批推荐项：disposition 一列四桶/t_fast 快照/rollup 只存计数/algorithm_id+MIXED/busbar_group.rated_capacity_kw/NULL=策略未生效；sql/vqms.sql 七八节 + migrations + S2S3StatsTablesIT 迁移自应用；⚠️ L1 跑批随 MYSQL_ROOT_PASSWORD；报告 docs/测试/S2_S3_统计落库测试报告_2026-08-24.md）；✅ S4 Slice1 已交付 2026-08-24（RegulationPipeline 三阶段装配体：ledger→门控→judge→免考/占位→vqms_regulation_cmd 幂等 upsert；主母线 yc4001 指示点解析；参数走 D7 缓存；stub 护栏拒写/门控不进分母/yx501 缺失从严按 0 三项默认口径待确认——报告 docs/测试/S4_Slice1管线装配测试报告_2026-08-24.md）；✅ S4 Slice2 已交付 2026-08-24（RuntimePipeline 投运率日记账：五信号整日预取内存阶跃保持→逐分钟分类计数→RuntimeStatistics 写回 vqms_runtime_daily；组合口径 max 并网≥10/原因从严/容量各组求和——报告 docs/测试/S4_Slice2_投运率日记账测试报告_2026-08-24.md）；✅ S4 Slice3 已交付 2026-08-24（VqmsStatsTask @vqmsStatsTask：recomputeYesterday 每日无参入口 + recomputeRange/recomputeRuntimeDay 手动回补；VqmsStatsRollupMapper 五语句三级 rollup 权威 SQL；sys_job 种子默认暂停防上线前静默产数——报告 docs/测试/S4_Slice3_Quartz接线测试报告_2026-08-24.md）。**S4 全部切片交付完毕、七项默认口径 2026-08-24 Leo 全部确认；✅ S5 工程侧（策略参数页）已交付 2026-08-24**（PolicyPreset 枚举 main 侧唯一权威 + VqmsPolicyParamService.applyPreset 写穿缓存 + /vqms/policyParam 控制器 perms vqms:policyparam:list/apply + 前端选套页四卡片三态横幅 + 菜单种子默认授予 role 2——报告 docs/测试/S5_策略参数页测试报告_2026-08-24.md；L3 人工冒烟待联调批）——✅ S4 读侧已交付 2026-08-24（VqmsAvcController 两端点接 rollup 表真数据：VqmsStatsQueryMapper + VqmsAvcStatsService 计数换算经 summarizeCounts 纯函数；daily/monthly/yearly 三端点维持占位待页面语义拍板）——搁置轨 S1~S4+S5 工程侧全落地，✅ 两枚 IT 2026-08-24 跑批通过（S1 准入 5/5 + S2S3 验收 6/6，证据入库）——**S1~S5 工程侧交付与验收全部闭环**；准入抓出免考旗逐档采样真实缺陷已修（yx501_fast/econ 双列，UNVERIFIED 待对端核对）；✅ **S5 选套已定并应用：乙（阈值剔除+计数，默认 50）2026-08-24 Leo 拍板**——四约定键写入实库（迁移 s5_apply_yi）、管线 disposition 链路接通、甲丙丁同权实现且一键切换经 L0 矩阵 + 实库 IT 双层验证（决策记录 docs/S5_政策选套与过渡期决策记录_2026-08-24.md；⚠ 乙边界方向/固定分母衔接两项 UNVERIFIED 待政策文件复核）；✅ 过渡期决策：真实样本暂以工具造数替代、点号核对维持合成点号+门控空转待现场核对后置 1；✅ daily/monthly/yearly 三页语义已拍板保留占位（选 B）；✅ 口令轮换完成（统一值，主库 ALTER+服务器 .env 同步）；余=乙两项 UNVERIFIED 随正式政策文件复核 + 真实样本回放（长期挂起，过渡期工具造数）+ Quartz 启用随上线拍板；开工蓝图 = v5.0 §8.8 解耦组件设计 + 测试方案 §5.0 组件测试矩阵，纯函数组件可即 TDD；S5 选套已拍板（乙，2026-08-24）、策略参数页可随时改选）**；另有 §5.1 真实数据回放验收（Leo 拍板的上线前置）。curve 消费模式债：LIMIT 501 + hasMore 收口，锚点=真实数据回放或统计解封。✅ 测试口令已轮换（2026-08-24 Leo 指定统一值）：主库 root@13306 已 ALTER 并同步服务器 .env，外部库 root@3306 同值；两枚 IT 与部署 .env 均已验证可用——口令值经会话/服务器 .env 传递，不入受跟踪文件。真实数据验证仍未做（等现场样本）。场景 IT/生成器窗口参数已于 2026-08-22 对齐 D7 锁定值 (t_fast=4, t_econ=5)（原拍板前旧口径 5/30 由 Leo D9 评审抓出，26 场景已重灌重算）。**✅ 戊·自由组合候选代码已落地（2026-08-25，Leo /goal 指令）**：策略文档 §3.3 草案（经两轮评审吸收）→ statistics 七纯函数类（PolicyAtom 原子 A1~A4+三归因/PolicyExpression 符号 canonical 可回读/FreeformPolicyParser 规则行 DSL「表达式->动作」同层混用须一层括号·嵌套上限一层/FreeformPolicyEvaluator 首中即断+兜底 COUNT_NORMAL(ruleId=null)/FreeformPolicyValidator fail-fast 含 COUNT_NORMAL↔A3 约束与 varchar(255) 守长/FreeformPolicyReducer 保守等价规约提示 ≡预设）+ Service 新键族 freeform_rule_001..N/freeform_threshold_pct（零 DDL，param_value varchar(255) 行式存储）+ applyFreeform/loadFreeformConfig（损坏显性 ISE 不静默降级）+ applyPreset 反向清理（单选生效）+ 管线自由组合优先路由 + 控制器 POST /vqms/policyParam/applyFreeform + 前端第五卡片与规则对话框；A5 免考旗子表未实现（随 §6.5 拍板）、命中规则 ID 列未加、分母口径冲突收窄校验随 §6-8 复核——L0 全绿 vqms 264 例（报告 docs/测试/戊自由组合_测试报告_2026-08-25.md，证据入库；Service 持久化路径归联调批）。**✅ 2026-08-26 收尾两件**：①Leo 拍板四套固定候选全部退役（策略文档 ⛔，甲乙丙丁零残留）→ 代码侧同步：选套页仅戊卡片（预设写路径前端入口删除、applyPreset 后端保留休眠）；②「命中规则 ID」留痕列落地（§3.3.5-4 ◐→✅）：hit_rule_id varchar(8) 迁移已对 ry_vqms 执行 + 管线 Disp(disposition,ruleId) 落账（戊命中=R001…，预设/兜底=NULL）+ 3 断言，回归 264+4 全绿（证据 docs/测试/证据/2026-08-25_规则ID留痕与候选屏蔽/）；页面部署归联调批。
