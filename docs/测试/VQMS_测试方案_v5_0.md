...
5. **S16（`L>H` 无效窗）契约承载未定（2026-08-19 评审吸收，自 §6-4「顺带」项提级为硬前置）**：并入 `Undecodable` 原因扩展 **vs** 独立无效信号——两选一须在 **D9 起草前拍板**，与 §6-1（thresholds.yaml 对齐）同列。迟拍/仓促拍的代价：§4.2 映射表该格与 `RegulationOutcome` 类型定义不同步改动 →「测试绿但契约没对齐」的**假绿**（阶段 0 收口项，§8）。
...
| 阶段 | 交付 | 测试铺开 |
|---|---|---|
| 0 | §6 前置差距收口（生成器对齐 + manifest 重算 + 循环码非法标注纪律 + S16 契约承载拍板（§6-5）） | L0（Python：对齐后的一致性校验，待建） |
