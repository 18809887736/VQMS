...
| 对象 | 打法 | 理由 |
|---|---|---|
| L0 纯函数（D3 时间对齐 / D4 闸门 / D9 策略评估 / 解码） | **严格 TDD**（先写测试→红→实现→绿→重构）+ **属性测试**（jqwik，见下） | 无 DB/网络依赖，循环毫秒级；D9 的甲/乙/丙/丁候选本身就是现成的参数化用例表 |
| L1/L2 集成与契约（D1 reader / D8 流水账 / L2 manifest 断言） | **契约先行**：断言批量先写、实现完成后跑——非逐步红绿 | 直连 10.0.0.9 网络往返使红绿循环变慢；manifest 断言天然是"先有的期望" |
...
- **值域**：`round(t)` 的秒/毫秒位归零、分钟能被 60 整除进位正确；
- 锚点：`29.999s` 舍 / `30.000s` 进 / 跨分跨小时跨天（示例枚不完的边界由属性生成器替你枚）。

**三个互补技术**（对示例式 TDD 的增强，非替代）：

1. **属性测试（jqwik，随 D3 引入）**：断言**不变量**而非个例，框架自动生成上千输入轰边界——D3（时间）、D4（正则闸门的正反例不变量：合法格式恒通过/非法格式恒拦截且不抛）、解码（编码→解码往返可逆）三处受益。
...
**前置**：严格 TDD 跑 manifest oracle 断言（L2 真实现档）前，§6 前置差距（thresholds.yaml 旧值 + manifest 重算）必须先收口——否则"红"是红的**假期望**。
