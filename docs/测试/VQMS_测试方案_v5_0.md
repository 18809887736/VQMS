...
- **类型契约**：`RegulationOutcome` sealed（`Judged{fast, econ, completeness, invalidTiers}` / `Undecodable{reason}`；invalidTiers = 按档无效标记，S16 L>H 承载，2026-08-19 拍板见 §6-5）；原因分类枚举三类（v5.0 §8.2：`循环码非法`/`缺t₀电压`/`编码脏写`）——**合成场景只覆盖两类**：S11=缺t₀电压、S12=编码脏写；`循环码非法` **须测试代码自造输入构造**（如 `9X02`，语义本身待生产数据验证，v5.0 §8.5/§14-9）——**自造用例一律在断言处显式标注 `UNVERIFIED-ASSUMPTION`（2026-08-19 评审吸收）**：它验证的是「对该错误分支行为的假设」而非真实行为，生产数据验证语义后须统一复核全部带标用例，防止被当成已验证契约依赖；S16 契约承载（§6-5）定稿后若同样零场景覆盖，其自造用例同标。
...
5. **✅ S16（`L>H` 无效窗）契约承载已定（2026-08-19 Leo 拍板）**：选 **B——`Judged` 内按档无效标记（`invalidTiers`），不并入 `Undecodable`**。理由：① 管线阶段对位——`Undecodable` = 解码失败，发生在取 `V_target` 之前；L>H 是窗口聚合**之后**才发现的数据异常（解码已成功）；② 归因纯净——`undecodable_count` 合规口径保持「解码失败归因」（编码侧的锅），采集侧脏数据不混入"谁的锅"分析；③ 粒度对齐——S16 是**按档**现象（fast 无效 + econ 正常合格）。策略层处置：与 completeness=0 同族的「判不了该档」事实。（历程：2026-08-19 评审自 §6-4「顺带」提级为硬前置，当日拍板，未留假绿窗口。）
