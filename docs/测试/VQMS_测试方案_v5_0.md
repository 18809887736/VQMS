...
- **L4 交叉比对**（非 CI）：在**真实 `qheatavchisdb`** 的同一批数据上，Java 读取结果 vs `verify/run_verify.py`（路 A）结果**双方**逐行比对，防 Java 与 Python 验证脚本行为漂移（真实库无 manifest 期望，不与 manifest 比对——manifest 期望的核对在 L1/L2 的合成库内做）。（2026-08-19 评审吸收：此层防漂移价值最高，D1 收尾即脚本化为一键比对，尽早从「人工/定时」转自动化。）
