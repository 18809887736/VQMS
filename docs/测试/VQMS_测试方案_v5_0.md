...
1. **属性测试（jqwik，随 D3 引入）**：断言**不变量**而非个例，框架自动生成上千输入轰边界——D3（时间）、D4（正则闸门：合法格式恒通过、非法格式恒拦截且不抛）、D9（策略评估：同输入同配置输出恒定、四套配置行为互斥且完备）三处受益。解码的「编码→解码往返可逆」属性随 S1 解封后作增强（Java 解码在 judge 内属搁置轨；Python `decode.py` 侧为既有 pytest 单测）。
...
- **属性断言（jqwik，§2.1）**：同输入同配置输出恒定；四套配置行为互斥且完备。
...
| CI | 暂无；**建议随 D1 落地本地 pre-commit hook 只跑 L0**（2026-08-19 评审吸收：无 CI 时维持红绿纪律不退化的最低成本手段）；将来上 CI 可恢复容器方案，测试代码不变只换 profile |
...
| 1 | D1 source 层 + D3 时间工具 + D4 闸门 | L0 全量（**严格 TDD，D3 起手 = jqwik 属性断言先写**，§2.1）+ L1（直连 `vqms_avc_test`）+ L4 交叉比对 |
