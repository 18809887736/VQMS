...
4. **`循环码非法` 分支零场景覆盖**：S12 只构造了编码脏写（`",abc."` 脏文本）一种失败，`循环码非法` 在 26 场景中没有任何数据——该类的 D9 构造用例**只能测试代码自造输入**（语义本身待生产数据，v5.0 §14-9）；自造用例的标注纪律见 §4.8（UNVERIFIED-ASSUMPTION）。
