...
**通用原则（2026-08-19 评审提炼，本方案两处已用）**：能用类型系统 / DB 约束**结构性排除**的缺陷，不用测试逻辑去堵——§4.8 sealed 不相交并（编译期排除不可达组合）、§4.7 uk 生成列候选（DB 层排除 TOCTOU 竞态）皆此式。
...
| 4 | D9 契约+策略骨架 | L0（纯函数四套候选参数化 + 循环码非法自造用例；TDD 严格红绿，§2.1）+ L1（策略参数表 DDL + 集成断言）+ L2（Stub 档契约形状，前置差距已收口）；收尾跑一次 **PIT** 评估护栏质量 |
