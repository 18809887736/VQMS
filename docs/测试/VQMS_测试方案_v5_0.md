...
3. **变异测试（PIT，后期一次性评估）**：D9 策略纯函数落地后跑一次——往代码注入变异（改边界、翻条件），看甲乙丙丁参数化测试能否杀死，检验护栏是真还是摆设；Maven 插件接入（§7），不作常驻门禁。
...
- **属性断言（jqwik，§2.1）**：① 同输入同配置输出恒定（纯确定性校验，零门槛）；② **任一输入在任一配置下恰好落一个处置桶（不重不漏）**——②的输入空间显式定义（2026-08-19 评审吸收预警：原「互斥且完备」措辞形式化门槛偏高，生成器需先有边界）：输入 = 窗口缺分钟比例 completeness ∈ [0,1] × Undecodable 归因 ∈ {无, 循环码非法, 缺t₀电压, 编码脏写} 的**笛卡尔积**（比例有限数值、归因有限枚举，无未定义边界）。
...
| 变异测试 | **PIT**（Maven 插件，D9 落地后一次性评估、不作常驻门禁，§2.1） |
