...
## 一、RuoYi 业务/系统表（ry_20260417.sql，22 张）
...
| `sys_dept` | 部门表 |
| `sys_role` | 角色表 |
| `sys_menu` | 菜单权限表（VQMS 业务菜单 vqms_menu.sql 追加于此） |
| `sys_role_menu` | 角色-菜单关联 |
| `sys_role_dept` | 角色-部门关联 |
| `sys_user_role` | 用户-角色关联 |
| `sys_user_post` | 用户-岗位关联 |
| `sys_post` | 岗位表 |
| `sys_dict_type` | 字典类型（含 VQMS 自建 `vqms_v_grade`） |
| `sys_dict_data` | 字典数据 |
| `sys_config` | 参数配置（验证码开关等，VQMS 关验证码 UPDATE 覆盖此处） |
| `sys_notice` | 通知公告 |
| `sys_notice_read` | 通知已读记录 |
| `sys_oper_log` | 操作日志 |
| `sys_logininfor` | 登录日志 |
| `sys_job` | 定时任务（Quartz 任务定义） |
| `sys_job_log` | 定时任务日志 |
| `gen_table` | 代码生成-业务表 |
| `gen_table_column` | 代码生成-业务表字段 |
